...
public class TinyIdInfoDAOImpl implements TinyIdInfoDAO {
    ...
    public TinyIdInfo queryByBizType(String bizType) {
        String sql = "select id, biz_type, begin_id, max_id," +
                " step, delta, remainder, create_time, update_time, reversion" +
                " from core_tiny_id_info where biz_type = ?";
    ...
    @Override
    public int updateMaxId(Long id, Long newMaxId, Long oldMaxId, Integer reversion, String bizType) {
        String sql = "update core_tiny_id_info set max_id= ?," +
                " update_time=now(), reversion=reversion+1" +
                " where id=? and max_id=? and reversion=? and biz_type=?";
        return jdbcTemplate.update(sql, newMaxId, id, oldMaxId, reversion, bizType);
    }
    ...
    public static class TinyIdInfoRowMapper implements RowMapper<TinyIdInfo> {
        ...
        public TinyIdInfo mapRow(ResultSet resultSet, int i) throws SQLException {
            ...
            tinyIdInfo.setReversion(resultSet.getInt("reversion"));
            return tinyIdInfo;
        }
    }
}
